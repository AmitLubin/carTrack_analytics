...
pipeline {
    ...
    stages {
        ...
        stage("Release-artifactory"){
            ...
            steps {
                script {
                    def telemetry = sh(script: "curl -u admin:Al12341234 -X GET 'http://artifactory:8082/artifactory/api/storage/libs-release-local/com/lidar/telemetry/1.0.0/'", returnStdout: true)
                    def simulator = sh(script: "curl -u admin:Al12341234 -X GET 'http://artifactory:8082/artifactory/api/storage/libs-snapshot-local/com/lidar/simulator/99-SNAPSHOT/'", returnStdout: true)
        ...
        stage('Test'){
            ...
            steps {
                ...
                sh "curl -u admin:Al12341234 -O 'http://artifactory:8082/artifactory/libs-release-local/com/lidar/telemetry/1.0.0/${JARTM}'"
                sh "curl -u admin:Al12341234 -O 'http://artifactory:8082/artifactory/libs-snapshot-local/com/lidar/simulator/99-SNAPSHOT${JARSIM}'"
                sh "ls -l"
                sh "ls target"
                sh "java -cp .${JARSIM}:.${JARTM}:target/analytics-${TAG}-SNAPSHOT.jar com.lidar.simulation.Simulator"
